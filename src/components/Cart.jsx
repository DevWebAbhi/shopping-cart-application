...
const Cart = () => {
  ...
  function removeItem(e) {
    ...
    dispatch({
      ...
      payload: selector.totalPrice - e.price * e.itemCount,
    });
    localStorage.setItem(
      "shoping-cart-assignment-cart-items-6",
      JSON.stringify(newData)
  ...
  return (
    <Box width={"90%"} margin={"auto"} paddingBottom={"5rem"}>
            ...
            >
              ...
              <Stack width={"100%"}>
                <CardBody>
                  ...
                  <Text py="2">{e.description}</Text>
                </CardBody>

                <CardFooter
                  display={isLargerThan450 ? "flex" : "block"}
                  width={"100%"}
      ...
      >
        <Text color="blue.450" mr={"1rem"} fontSize="2xl" display={"flex"}>
          ...
          {" : "} {Number(selector.totalPrice).toFixed(2)}
        </Text>
