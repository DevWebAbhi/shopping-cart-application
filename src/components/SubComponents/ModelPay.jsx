...
function ModelPay({price}) {
  ...
  const payNow = (onClose)=>{
    if(ref.current === "1235" ){
      ...
      localStorage.setItem("shoping-cart-assignment-cart-items-6",JSON.stringify([]));
  ...
  return (
    <>
      <Button variant='solid' colorScheme='green' onClick={onOpen}>Pay Now</Button>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody textAlign={"center"}>
            <Text  mb={"1.5rem"} color='blue.450' fontSize='2xl'>Enter Test PIN 1235</Text>
            <Text>Grand Total {" : $"+price}</Text>
          </ModalBody>
          <HStack width={"max-content"} margin={"auto"} mt={"3rem"}>
            <PinInput otp onChange={(e)=>{
              ref.current=e;
            }}>
              <PinInputField />
              <PinInputField />
